\section{Introduction and Research Area}

The purpose of this thesis was to design, develop, and deploy Smart Travel, a fullstack, cloud-native web platform that empowers travel agencies to compose, market, and sell custom travel packages, while allowing customers to easily search for flights, accommodations, and activities. The primary objective was to solve the main challenges of distributed computing --- namely data consistency across microservices, secure inter-service communication, network latency, and deployment automation. The solution implements state-of-the-art industry patterns such as the Backend-For-Frontend (BFF), the Transactional Outbox pattern, Change Data Capture (CDC), and Continuous Integration/Continuous Delivery (CI/CD) pipelines via Kubernetes-based orchestration.

\section{System Architecture Overview}

To achieve strict separation of concerns, the platform was decomposed into distinct microservices, each responsible for a specific domain:
...
\begin{itemize}
    \item \textbf{Database:} To guarantee decoupled persistence, each microservice operates with its own isolated \textbf{MongoDB} database. 
    \item \textbf{Messaging Layer:} For asynchronous, event-driven communication, the system utilizes \textbf{RabbitMQ} as the central message broker, working in conjunction with a \textbf{Debezium} service for Change Data Capture (CDC).
    \item \textbf{Frontend Application:} The client-facing interface is a Single Page Application built with \textbf{Angular} and \textbf{PrimeNG}. It communicates exclusively with the BFF via GraphQL.
\end{itemize}
...
\section{The BFF and Distributed Data Consistency}

To prevent frontend coupling and over-fetching, a \textbf{BFF} was implemented using Spring for GraphQL. It dynamically aggregates downstream data and serves as the primary security perimeter, enforcing Role-Based Access Control (RBAC) using JSON Web Tokens (JWT).

The most complex challenge was maintaining data consistency during the financial checkout phase. To solve this, the platform implements the \textbf{Transactional Outbox Pattern} combined with \textbf{Change Data Capture (CDC)}. When a user completes a payment, the event is saved atomically into MongoDB. A standalone \textbf{Debezium} service scans the database operations log (\textit{oplog}) and publishes the event to RabbitMQ. Finally, the Notification API asynchronously consumes the event, guaranteeing at-least-once message delivery.

\section{Infrastructure Automation and Cloud Deployment}

To operationalize the platform, deployment was fully automated via \textbf{Jenkins Declarative Pipelines}. Ephemeral Docker build agents dynamically compile, containerize, and securely push optimized images to an internal \textbf{OKD (OpenShift)} ImageStream registry. The workloads are orchestrated via Kubernetes manifests (\texttt{Deployments}, \texttt{ConfigMaps}, \texttt{Secrets}). For security, the internal microservices are hidden from the public internet; instead, the Nginx frontend container acts as a secure Reverse Proxy routing traffic to the internal cluster network.
